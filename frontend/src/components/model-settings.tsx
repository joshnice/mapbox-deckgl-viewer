...
interface ModelSettingsProps {
	...
	onValidationTestingClicked: () => void;
			...
			{showOptions && (
				<div className="model-setting-items">
					...
					<div className="model-setting-item testing-button">
						<button
							disabled={testing || (singleModelTest && singleModelTestAmount === 0)}
							onClick={handleTestingClicked}
						>
							Start Testing
						</button>
						<button disabled={testing ?? false} onClick={onValidationTestingClicked}>
							Validation test
						</button>
					</div>
					<button className="change-model-button" disabled={testing ?? false} onClick={onChangeModelClick}>
						Change Model
					</button>
				</div>
			)}
